...
public class IndexHTML {
  ...
  private static TermEnum uidIter;		  // document id iterator
   private static final Logger logger = LoggerFactory.getLogger(IndexHTML.class);
  /** Indexer for HTML files.*/
  public static void main(String[] argv) {
    try {
      File index = new File("indexHTML");
      boolean create = true;
      File root = null;
...
//      }
      root = new File(IndexHTML.class.getClassLoader().getResource("org").getPath());         // --1
      if(root == null) {
      ...
      if (uidIter != null) {
        ...
        while (uidIter.term() != null && uidIter.term().field() == "uid" &&
          ...
          if (deleting) {			  // delete stale docs
            logger.info("deleting " + HTMLDocument.uid2url(uidIter.term().text()));
            reader.deleteDocuments(uidIter.term());
          }
          uidIter.next();
        }
